...
export const isAuthenticatedUser = async(req, res, next) => {
    try {
        const {token}=req.cookies;//dikkat to yahi hai  
        console.log("isAuthUserToken check",token);
        if(!token)
        {
...
export const AuthenticatedRole = (...roles) => {
  try {
      return (req, res, next) => {
        if (!roles.includes(req.user.role)) {  //roles array mai agar admin nahi hai to unauthorised else continue ir next()
          return next(
            new ErrorHandler(
